...
switch($_POST['type'])
{
    ...
    case "import_file_format_keepass":
        ...
        //prepare CACHE files
        $cacheFile_name = $_SESSION['settings']['cpassman_url']."/files/cpassman_cache_".md5(time().mt_rand());
        $cacheFile_name_folder = $cacheFile_name."_folders";
        $cacheFile = fopen($cacheFile_name,"w");
        $cacheFileF = fopen($cacheFile_name_folder,"w");

        //read xml file
        $xml = simplexml_load_file($_SESSION['settings']['cpassman_url']."/files/".$_POST['file']);
...
echo $_SESSION['settings']['cpassman_url']."/files/".$_POST['file'];
